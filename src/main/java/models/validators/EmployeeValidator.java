...
public class EmployeeValidator {
     ...
     * @param servive 呼び出し元Serviceクラスのインスタンス
     * @param ev EmployeeServiceのインスタンス
     * @param codeDuplicationCheckFlag 社員番号の重複チェックを実施するかどうか(実施する：true、実施しない：false)
     * @param passwordCheckFlag パスワードの入力チェックを実施するかどうか(実施する：true、実施しない：false)
     * @return エラーのリスト
     */

    public static List<String> validate (
            EmployeeService service, EmployeeView ev, Boolean codeDupulicateCheckFlag, Boolean passwordCheckFlag ) {
        List<String> errors = new ArrayList<String>();

        //社員番号のチェック
        String codeError = validateCode(service, ev.getCode(), codeDupulicateCheckFlag);
        if (!codeError.equals("")) {
     ...
     * @param passwaordCheckFlag パスワードの入力チェックを実施するかどうか(実施する：true、実施しない：false)
     */
