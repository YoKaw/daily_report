...
public class ReportService extends ServiceBase {
    ...
    /**
     * 指定した従業員が作成した日報データを、指定されたページ数の一覧画面に表示する分を取得し、ReportViewのリストで返却する
     * @param employee 従業員
     * @param page ページ数
     ...
     * @return 日報データの件数
     */
    public long countAllMine(EmployeeView emoloyee) {

        long count = (long) em.createNamedQuery(JpaConst.Q_REP_COUNT_ALL_MINE, Long.class)
                .setParameter(JpaConst.JPQL_PARM_EMPLOYEE, EmployeeConverter.toModel(emoloyee))
                .getSingleResult();
    ...
    /**
     * 指定されたページ数の一覧画面に表示する日報データを取得し、ReportViewのリストに返却する
     * @param page ページ数
     * @return 一覧画面に表示するデータのリスト
     */
    ...
    public List<String> create(ReportView rv) {
        ...
        //バリデーションで発生したエラーを返却する(エラーがなければ0件の空のリスト)
        return errors;
    }
